...
export default function BulkUploadForm({ isEdit = false, isView = false, currentRole, isLoading }) {
  ...
  const onSubmit = async (data) => {
    try {
      const response = await dispatch(
        isEdit ? updateRoleAsync({ id: currentRole?._id, data }) : addRoleAsync(data)
      );
