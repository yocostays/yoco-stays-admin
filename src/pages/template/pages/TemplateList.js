...
const TABLE_HEAD = [
  ...
  { id: 'hostelName', label: 'Hostel', align: 'left' },
...
export default function TemplateList() {
  ...
  const { isLoading, getTemplateList, totalCount } = useSelector((store) => store?.template);
  ...
  const handleDeleteRow = async (row, closeModal) => {
    ...
    if (getTemplateList?.length === 1 && query?.page > 1) {
      setQuery((p) => {
        p.page -= 1;
        return { ...p };
      });
  ...
  const handleDeleteRows = (selectedRows) => {
    ...
    setSelected([]);
    if (query?.page > 1) {
      if (selectedRows?.length === getTemplateList?.length) {
        setQuery((p) => {
          p.page -= 1;
          return { ...p };
        });
      } else if (getTemplateList?.length > selectedRows?.length) {
        dispatch(getTemplateListAsync(query));
      }
    } else {
      dispatch(getTemplateListAsync(query));
  ...
  const handleEditRow = (row) => {
    navigate(PATH_DASHBOARD.template.edit(row?._id), { state: row });
  };
      ...
      <Container maxWidth={themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: 'Template', href: PATH_DASHBOARD.template.list },
            { name: 'List' },
          ]}
          action={
           <Box sx={{ display: "flex" }}>
        ...
        <Card>
          ...
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            <TableSelectedAction
              ...
              rowCount={getTemplateList?.length}
              onSelectAllRows={(checked) =>
                onSelectAllRows(
                  checked,
                  getTemplateList?.map((row) => row?.id)
                )
              }
              action={
            ...
            <Scrollbar>
              <Table size={dense ? 'small' : 'medium'} sx={{ minWidth: 800 }}>
                ...
                <TableBody>
                  ...
                  <TableEmptyRows
                    // height={denseHeight}
                    emptyRows={getTemplateList?.length ? query.limit - getTemplateList.length : 0}
                  />
